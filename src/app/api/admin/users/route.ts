...
import { getSessionFromRequest, getUsersByEmpresa, addUser, updateUser, getUserById } from '@/lib/dataStore';
...
export async function GET(req: NextRequest) {
  ...
  if (session?.role !== 'admin') return NextResponse.json({ error: 'No autorizado' }, { status: 401 });

  const users = (await getUsersByEmpresa(session.empresaSlug))
...
export async function POST(req: NextRequest) {
  ...
  if (action === 'create') {
    ...
    const existing = (await getUsersByEmpresa(slug)).find(u => u.username === username);
    if (existing) return NextResponse.json({ error: `El usuario "${username}" ya existe`, username }, { status: 409 });

    const defaultPassword = `${slug}2026`;
    await addUser({
      id: randomUUID(), username, password: defaultPassword,
      displayName: `${firstName} ${lastName}`,
      role: 'participant', empresaSlug: slug, active: true,
  ...
  if (action === 'toggle') {
    ...
    if (!user || user.empresaSlug !== slug) return NextResponse.json({ error: 'Usuario no encontrado' }, { status: 404 });
  ...
  if (action === 'resetPassword') {
    ...
    if (!user || user.empresaSlug !== slug) return NextResponse.json({ error: 'Usuario no encontrado' }, { status: 404 });
    const defaultPassword = `${slug}2026`;
    await updateUser(id, { password: defaultPassword });
